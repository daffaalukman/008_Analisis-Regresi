---
title: "Praktikum 2 Anreg by Daffa Lukman Wahid Al Jailani"
author: "Daffa Lukman"
date: "`r Sys.Date()`"
output:
  html_document:
...
data <- read_excel("C:\\Users\\Lenovo\\Downloads\\NSC-2025.xlsx", sheet="Sheet2")
...
```{r}
plot(data$X2, data$Y,
     main = "PDRB ADHK (MILYAR RUPIAH) vs Indeks Pembangunan Manusia",
     xlab = "PDRB ADHK",
     ylab = "IPM")
```
...
\hat{Y_i} = \hat{\beta_0} + \hat{\beta_1} X_{2i}, \quad i = 1,2,\ldots,n, i = 1,2, ..., n$
...
$$\hat{Y} = 7.426e+01 + 2.295e-05 X_2$$

dengan Y adalah Indeks Pembangunan Manusia (IPM) dan X adalah PDRB ADHK (miliar rupiah).

Didapatkan nilai $\hat\beta_0 = 7.426e+01$ artinya nilai dugaan Indeks Pembangunan Manusia (IPM) ketika PDRB ADHK bernilai nol adalah sebesar 74,26. Namun, karena nilai PDRB ADHK = 0 tidak terdapat dalam selang pengamatan, maka $\hat\beta_0$ tidak memiliki interpretasi ekonomis yang bermakna.

Nilai $\hat\beta_0 = 2.295e-05$ artinya setiap kenaikan PDRB ADHK sebesar 1 miliar rupiah akan meningkatkan rataan IPM sebesar 0,00002295 satuan, dengan asumsi faktor lain konstan.

Koefisien determinasi yang diperoleh sebesar 0,8884 (88,84%), yang berarti bahwa 88,84% keragaman Indeks Pembangunan Manusia (IPM) dapat dijelaskan oleh PDRB ADHK (X₂).
Sisanya sebesar 11,16% dijelaskan oleh faktor lain di luar model.
